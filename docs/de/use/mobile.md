...
2. **Widgets** wählen
3. **Mindwtr** suchen und entweder das Widget **Aufgaben** oder die einzellige Schaltfläche **Schnellerfassung** hinzufügen. Das Aufgaben-Widget zeigt den Eingangszähler, eine **+**-Schaltfläche und Ihre Aufgaben, jeweils mit Prioritätsfarbe, Projekt oder Bereich und Fälligkeitsdatum. Auf Android öffnet sich beim Platzieren des Widgets eine Auswahl: Wählen Sie Fokus (Standard, mit denselben Abschnitten wie der Fokus-Bildschirm: Heutiger Fokus, Heute, Durchsicht fällig, Nächste Aktionen, Anstehend, in der Standard-Fokusreihenfolge und ohne Filter), Eingang, Nächste Aktionen, Warten auf, Irgendwann/Vielleicht oder ein einzelnes Projekt, dessen Abschnitte zu Überschriften werden. Um die Liste später zu ändern, drücken Sie lange auf das Widget; bietet Ihr Launcher nur Umbenennen und Entfernen an, entfernen Sie das Widget und fügen Sie es erneut hinzu. Sie können mehrere Aufgaben-Widgets mit verschiedenen Listen hinzufügen.
