...
| 平台 | 安裝方式 |
| --- | --- |
| **Arch Linux** | 使用預先建置的 AUR 套件 `mindwtr-bin`（另有社群維護的 `mindwtr` 原始碼套件） |
...
全新安裝後，第一個選項是**開始使用 Mindwtr**。它會以你的語言建立**快速上手**專案並立即開啟。你也可以匯入現有任務、連接同步，或略過引導。記錄任務不需要先設定同步或匯入。

手機與桌面版的快速上手專案都提供直接記錄到**收集箱**、開啟收集箱進行處理，以及開啟**專注**的操作入口。專案、專注與日期的簡短提示會出現在相關位置。關閉提示後，仍可透過**說明**再次展開，並閱讀對應章節的指南。提示的關閉狀態只保存在目前裝置，不會隨任務同步。

之後啟動時，Mindwtr 預設開啟**專注**。需要迅速記下一個想法時，隨時將新工作收進**收集箱**。
...
| `/priority:<level>` | `Task /priority:high`     | 設定優先順序（`low`、`medium`、`high`、`urgent`） |
...
**即時預覽**：在桌面版或行動版的任何收集輸入欄中輸入時，輸入欄下方會以方塊顯示 Mindwtr 辨識到的內容：解析出的日期、專案、領域、情境、標籤、指派對象與狀態；若某個日期指令無法辨識，還會出現一個警告方塊。在辨識到內容之前方塊不會顯示；方塊與儲存時使用的是同一次解析，因此任務會完全依照方塊顯示的內容建立。
