...
声明了自身颜色的日历源会保留这些颜色：Mindwtr 会读取标准的 `COLOR` 属性、Apple 的日历颜色，以及按类别设置的颜色（例如 Fossify 的），因此订阅的日历看起来和它来源应用中一样。你在 Mindwtr 中自己选择的颜色始终优先于源提供的颜色；没有任何颜色信息的日历会获得自动分配且固定不变的颜色。
