...
La bêta Flathub nécessite la branche bêta et les autorisations dans `flathub/tech.dongdongbh.mindwtr`. Les versions stables publient les paquets AUR `mindwtr-bin` et `mindwtr` depuis `release.yml` après une validation dans un conteneur propre et un contrôle de propriété. Les builds RC publient `mindwtr-beta-bin` par `update-aur-beta.yml` avec les mêmes contrôles de sécurité. Si l’AUR désactive les poussées pendant une maintenance, relancez ce canal lorsqu’elles reprennent. Le flux `publish-aur.yml`, protégé par l’Environment `aur-publish`, récupère uniquement une proposition vérifiée pour le paquet source stable `mindwtr` ; il ne publie pas les paquets binaires ou bêta.
