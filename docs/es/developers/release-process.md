...
- AUR `mindwtr-bin-beta` se compila y valida cuando existen los recursos del prelanzamiento de GitHub; después, sus archivos exactos `PKGBUILD` y `.SRCINFO` se guardan como artefacto para revisión. El flujo de RC no envía cambios a AUR.
...
La beta de Flathub requiere la rama beta y permisos en `flathub/tech.dongdongbh.mindwtr`. Los lanzamientos estables publican los paquetes de AUR `mindwtr-bin` y `mindwtr` desde `release.yml` después de validarlos en un contenedor limpio y comprobar la propiedad. El flujo guarda como artefacto el árbol exacto del paquete fuente. Las compilaciones RC mantienen `mindwtr-bin-beta` como propuesta; usa el flujo de recuperación manual `publish-aur.yml`, protegido por el Environment `aur-publish`, cuando AUR acepte envíos. El mantenimiento de AUR puede retrasar este canal sin hacer fallar el lanzamiento de Mindwtr.
...
| T-3 | Confirma el prelanzamiento de GitHub de `release-rc.yml`, verifica la solicitud de incorporación de cambios de la beta de Flathub y la propuesta guardada de `mindwtr-bin-beta` cuando esas entradas estuvieran habilitadas, y anuncia la RC a quienes vayan a probarla. |
