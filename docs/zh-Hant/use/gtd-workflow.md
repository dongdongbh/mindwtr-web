...
- **開始日期**是延後／可用性的門檻。未來的開始日期預設會在「專注」中隱藏任務。日期到來時，任務會以原有狀態再次出現。如果開始日期帶有具體時間，「專注」和下一步行動會將任務隱藏到那個時刻——下午 5 點才能開始的任務不會一早就佔據清單。未來 7 天內的開始日期（包含今天稍後開始的）仍會在「專注」的**即將到來**區塊中預覽，因此延後到期時絕不會毫無預警。
...
4. **即將到來**預覽目前被延後規則擋住、但會在未來 7 天內出現的 `next` 任務：可能是未來的開始日期，也可能是等待下一個到期或回顧日期的重複任務。開始時間在今天稍後的任務也列在這裡，專注會把它隱藏到那個時刻。這些項目依將要出現的日子排序並顯示該日期。今天稍後開始的項目可以加星號加入今日專注——星號會保留，任務到開始時間時在那裡重新出現——而延到其他日期的則不行。沒有任務即將到來時此區塊也會消失。
