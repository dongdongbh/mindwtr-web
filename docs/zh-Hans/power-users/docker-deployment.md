...
2. **在旁边创建 `.env` 文件**（Docker Compose 会自动读取）：
   ...
   `MINDWTR_CLOUD_CORS_ORIGIN` 必须与你在浏览器中打开 PWA 的地址完全一致，包括协议和端口。`http://localhost:5173` 仅在浏览器运行于 Docker 主机上时有效，从其他设备访问时请填写主机地址，例如 `http://192.168.1.20:5173`。只能设置一个来源。
...
对于公共 URL、自定义 DNS 名称、VPN 主机名、Tailscale、ZeroTier，或任何未被识别为本地/私有的名称，请使用 HTTPS，或在应用的同步设置中开启**允许不安全连接（HTTP）**，以便通过明文 HTTP 接受该主机名。开启后数据将以未加密方式传输，请仅在可信网络中使用。
...
任务提醒以及开始、截止和复查日期提醒在浏览器版本中同样会触发，形式为网页通知。请为站点授予通知权限（设置 → 通知），并保持一个 Mindwtr 标签页处于打开状态：浏览器版本没有后台服务，因此只有在标签页运行时才会送达提醒。位于后台的标签页会被限制计时器，提醒可能迟到最多一分钟；在所有标签页都关闭时到点的提醒会被跳过，而不会稍后补发。若需要在浏览器关闭后仍然可靠的提醒，请使用桌面或移动应用。
