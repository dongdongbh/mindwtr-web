...
Years of normal task use lands in the hundreds of kilobytes to a few megabytes, small next to the `attachments/` folder if you attach files or audio. If the snapshot feels large: empty the Trash, run attachment cleanup in **Settings → Data**, and check `attachments/` before worrying about the JSON.
