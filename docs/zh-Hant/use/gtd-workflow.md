...
- **開始日期**是延後／可用性的門檻。未來的開始日期預設會在「專注」中隱藏任務。日期到來時，任務會以原有狀態再次出現。如果開始日期帶有具體時間，「專注」和下一步行動會將任務隱藏到那個時刻——下午 5 點才能開始的任務不會一早就佔據清單。未來 7 天內的開始日期仍會在「專注」的**即將到來**區塊中預覽，因此延後到期時絕不會毫無預警。
...
4. **即將到來**預覽目前被延後規則擋住、但會在未來 7 天內出現的 `next` 任務：可能是未來的開始日期，也可能是等待下一個到期或回顧日期的重複任務。這些項目依將要出現的日子排序並顯示該日期；它們只是預覽，因此無法加星號加入今日專注，沒有任務即將到來時此區塊也會消失。
