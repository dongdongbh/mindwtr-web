...
- `contentRev` und `fileHash` bilden die synchronisierte Inhaltsidentität der angehängten Datei. Ein Gerät erhöht `contentRev` erst, wenn ein Hash bestätigt, dass sich die Bytes wirklich geändert haben. Ein höherer `contentRev` gewinnt daher die Inhaltsfelder eindeutig, selbst wenn die andere Seite die Zusammenführung auf Anhangsebene gewinnt; bei Gleichstand entscheidet deren Gewinner. Ein fehlender `contentRev` zählt als `0`, sodass Clients ohne Inhaltsverfolgung sauber zusammengeführt werden.
- Die aufgezeichnete Änderungszeit und Größe der Datei bleiben gerätelokal und werden vor jedem entfernten Schreibvorgang entfernt. Sie beschreiben die Kopie der Bytes auf diesem Gerät, und jedes Gerät leitet sie nach dem eigenen Hoch- oder Herunterladen neu ab. Genau weil sie nicht im synchronisierten Datensatz stehen, kann ein Synchronisierungslauf eine lokal bearbeitete Datei per Statusvergleich erkennen, ohne in jedem Lauf eine entfernte Änderung zu schreiben.
