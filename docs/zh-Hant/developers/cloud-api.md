...
| `all=1` | 納入已完成任務。 |
| `deleted=1` | 納入軟刪除任務。 |
| `isFocusedToday` | `true`/`1` 回傳加星號任務，包括排隊等待進入專注的未來開始任務；`false`/`0` 回傳其餘任務。省略則不篩選。 |
...
`assignedTo` 是單一人員姓名的字串，例如 `"Alex"`，不是人員 ID 陣列。使用 `null` 或 `""` 清除指派。透過 REST 建立或更新任務時，陣列、物件、數字和布林值會被拒絕，並傳回 HTTP 400。搜尋會忽略已儲存的非字串指派值，不會改寫原始資料。
