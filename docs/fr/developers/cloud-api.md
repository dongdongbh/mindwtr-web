...
| `isFocusedToday` | `true`/`1` renvoie les tâches étoilées, y compris les actions suivantes à début futur en attente de Focus ; `false`/`0` renvoie les autres. Omettez-le pour ne pas filtrer. |
...
`assignedTo` contient le nom d’une seule personne sous forme de chaîne, par exemple `"Alex"`, et non un tableau d’identifiants de personnes. Utilisez `null` ou `""` pour supprimer l’affectation. La création et la modification de tâches via REST rejettent les tableaux, objets, nombres et booléens avec HTTP 400. La recherche ignore les affectations déjà stockées dont le type est incorrect, sans les réécrire.
