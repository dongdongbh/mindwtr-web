...
> API 密钥、自定义 AI 端点 URL（包括语音转文字端点）、额外的请求参数和本地模型路径绝不会同步。自定义端点需要在每台设备上分别填写一次。
...
在 Android 上，请为文件同步选择设备上的本地文件夹。若要使用 Google Drive，请通过单独的文件夹同步应用将云端文件夹镜像到该本地文件夹，然后在 Mindwtr 中选择这个本地文件夹；请勿直接选择 Google Drive 云端位置。请参阅上文的 **Android 上的 Google Drive（文件同步）和 Dropbox 文件同步回退方案**。Dropbox 或 WebDAV 是可直接连接的替代方案。

### 1. 先导出数据

1. 前往**设置 → 数据**
2. 轻点**导出备份**
3. 将文件保存到同步文件夹。在 Android 上，请使用之后将在 Mindwtr 中选择的同一个设备本地文件夹
...
- **自动导入**：选定列表后，此开关会在每次打开 Mindwtr 或回到前台时运行同样的导入，这样随手记下的提醒事项无需手动点击即可进入收集箱。iOS 不允许应用在后台读取提醒事项，因此只在应用打开时检查。导入了内容时会显示一条简短提示。
