...
1. 安裝 F-Droid 用戶端。
2. 開啟 [F-Droid 上的 Mindwtr](https://f-droid.org/en/packages/tech.dongdongbh.mindwtr/)並安裝。
...
2. 開啟 [IzzyOnDroid 上的 Mindwtr](https://apt.izzysoft.de/fdroid/index/apk/tech.dongdongbh.mindwtr)並安裝。
...
從下一個 App Store 穩定版開始，iOS App 將包含已在配對實體手錶上測試的原生 Apple Watch App。將執行 watchOS 10 或更新版本的 Apple Watch 與 iPhone 配對，安裝 Mindwtr，然後透過 iPhone 上的 Watch App 將其安裝到手錶。點一下**說話**可錄製音訊並由 iPhone 轉寫，點一下**輸入**可鍵入文字。你也可以完成或延後專注任務，並控制連結的番茄鐘。
...
為了確保 Mindwtr 能持續存在並繼續開發，非常感謝你的支持。如果這款應用程式對你有幫助，請考慮透過 [GitHub Sponsors](https://github.com/sponsors/dongdongbh) 或 [Ko-fi](https://ko-fi.com/D1D01T20WK) 支持本專案。
