...
- `contentRev` y `fileHash` son la identidad de contenido sincronizada del archivo adjunto. Un dispositivo solo incrementa `contentRev` después de que un hash confirme que los bytes cambiaron de verdad, así que un `contentRev` mayor gana los campos de contenido de forma directa aunque el otro lado gane la fusión a nivel de archivo adjunto; en caso de empate decide ese ganador. Un `contentRev` ausente cuenta como `0`, de modo que los clientes anteriores al seguimiento de contenido se fusionan sin problemas.
- La fecha de modificación y el tamaño registrados del archivo son locales del dispositivo y se eliminan antes de cualquier escritura remota. Describen la copia de los bytes de este dispositivo, y cada dispositivo vuelve a derivarlos tras su propia subida o descarga. Mantenerlos fuera del registro sincronizado es lo que permite que un ciclo de sincronización detecte un archivo editado localmente comparando esos datos sin escribir un cambio remoto en cada ciclo.
