...
3. Trouvez **Mindwtr** et ajoutez soit le widget **Tâches**, soit le bouton d’une cellule **Capture rapide**. Le widget Tâches affiche le nombre d’éléments dans la boîte de réception, un bouton **+** et vos tâches, chacune avec sa couleur de priorité, son projet ou domaine et sa date d’échéance. Sous Android, un sélecteur s’ouvre lorsque vous placez le widget : choisissez Focus (par défaut, avec les mêmes sections que l’écran Focus : Focus du jour, Aujourd’hui, Revue due, Prochaines actions, À venir, dans l’ordre Focus par défaut et sans filtre), Boîte de réception, Prochaines actions, En attente, Un jour/Peut-être ou un projet, dont les sections deviennent des en-têtes. Pour changer de liste plus tard, appuyez longuement sur le widget ; si votre lanceur ne propose que Renommer et Supprimer, supprimez le widget et ajoutez-le à nouveau. Vous pouvez ajouter plusieurs widgets Tâches avec des listes différentes.
