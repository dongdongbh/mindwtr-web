...
使用**设置 → 数据 → 导入数据 → 导入 Mindwtr CSV**，可导入文档规定的任务表格或包含 CSV 文件的 ZIP。格式涵盖状态、项目、分区、领域、情境、标签、负责人、优先级、精力、开始/截止/回顾日期、完成状态、清单和手动顺序。重新导入 Mindwtr 导出时，已有任务 ID 会被跳过，不会重复或覆盖。
...
Mindwtr 不提供自己的加密层；静态数据的保护取决于持有数据的设备、服务器或服务商：
...
- **Dropbox 和 WebDAV**：传输使用 TLS，服务商通常也会加密其存储，但密钥在服务商手中，技术上可以读取同步文档。
- **自托管云端**：隐私取决于服务器的运维方式。VPN 只保护访问和传输；要保护磁盘上的副本，仍需对服务器磁盘加密（例如 LUKS）。

如今若需要加密的同步目标，可将文件夹同步指向由其他工具加密的文件夹——加密文件系统或 gocryptfs/Cryptomator 挂载——或搭配 Syncthing 的不受信任设备模式，让中转节点只保存密文（Syncthing 的测试版功能；受信任设备仍保留可读副本）。
