...
La beta de Flathub requiere la rama beta y permisos en `flathub/tech.dongdongbh.mindwtr`. Los lanzamientos estables publican los paquetes de AUR `mindwtr-bin` y `mindwtr` desde `release.yml` después de validarlos en un contenedor limpio y comprobar la propiedad. Las compilaciones RC publican `mindwtr-beta-bin` mediante `update-aur-beta.yml` con las mismas comprobaciones de seguridad. Si AUR desactiva los envíos durante el mantenimiento, vuelve a ejecutar ese canal cuando los habilite. El flujo `publish-aur.yml`, protegido por el Environment `aur-publish`, solo recupera una propuesta revisada del paquete fuente estable `mindwtr`; no publica los paquetes binarios ni beta.
