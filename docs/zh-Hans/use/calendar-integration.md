...
如水支持两个方向的日历集成：将外部日历读入规划界面，以及在平台提供可写设备日历时，将如水任务推送到其中。

- **移动端（iOS/Android）：**系统已提供的设备日历、ICS 订阅 URL，以及从如水到设备日历的单向推送
...
- **软景观（Soft Landscape）**：设置了 `startTime` 和 `timeEstimate` 的如水任务。
...
重复如水任务会生成为任务实例，而不是展开成日历系列：

- 当活跃任务实例有截止日期或计划开始时间时，日历会显示该实例。
- 默认情况下，如水不会在日历中预先填充未来的任务实例。只有当前重复任务完成后，才会创建下一个真实任务实例。
- 如果为重复任务启用了**在日历中显示下一次**，日历可以显示下一次发生时间的一个仅供规划的预览。该预览不是真实任务；移动端/macOS 日历推送会将其写为普通的单次事件，而不是原生重复事件。
- **严格**会保持固定的计划节奏。每月 1 日到期的月度任务会继续锚定在该计划周期，但如水仍只会在每次完成时创建一个下一实例，而不会填满未来的每个月。
- **完成后重复**会根据你完成当前任务的时间计算下一实例。例如，一个普通的月度习惯在 15 日完成，下次就会从 15 日起安排。
- 移动端和 macOS 日历推送会镜像这些具体任务实例。它们不会把如水的重复规则导出为原生日历重复事件。
...
3. 设置开始和结束时间。如水会对照已排期任务和可见的外部事件检查该时间段。
...
1. 从日视图打开**日历 -> 安排任务**。
2. 选择一个现有任务。
3. 如水会查找当天最早的空闲时间段，并避开可见的外部事件和已排期任务。

移动端排期面板的用法相同：当你已经在查看某一天时，用它选择尚未排期的工作，然后分配具体的开始时间。

如有 `timeEstimate`，如水会将其用作默认时长。如果有冲突，请选择其他时间或缩短时长。
...
| 平台 | 支持的日历功能 | 说明 |
| --- | --- | --- |
| iOS/Android 移动端 | 将如水任务推送到设备日历 | Android 已通过 Google Calendar 验证。在 iOS 上，请使用 Apple Calendar/EventKit 已经可以访问的日历。 |
...
| macOS 桌面端 | 将如水任务推送到 Apple Calendar | 通过 EventKit 将已排期/到期的如水任务写入选定的可写 Apple Calendar 目标。 |
...
- 在如水中进行日历提供商 OAuth，例如从如水登录 Google、Microsoft 或 Nextcloud。
- 返回 `HTTP 401` 的需认证/私有 URL，除非日历提供商已将密钥嵌入 URL。
- 日历网页 URL，包括渲染 HTML 而非原始 `.ics` 数据的公开分享页面。
- 从如水编辑外部日历事件。
- 通过如水同步来同步外部日历事件。外部事件会在本地获取和缓存。
- 双向任务/日历同步。推送的日历事件由如水任务生成。
- 将重复任务规则导出为原生重复日历事件。
...
在 iOS 和 Android 上，如水可以将已排期任务和带截止日期的任务推送到选定的设备日历：

- 带具体时间的 `startTime` 任务会成为定时事件。如有 `timeEstimate`，它会用作事件时长。
- `startTime` 仅含日期或只有 `dueDate` 的任务会成为全天事件。
- 已完成、已归档、参考资料或已删除的任务会从推送目标日历中移除。
- 推送到专用的 `Mindwtr` 日历时，如水会保留任务标题。如果你选择共享日历目标，推送的事件标题会加上 `Mindwtr: ` 前缀，以便与普通事件并列时仍可识别。
- 任务描述会成为事件备注，任务地点会成为事件地点。
- 如果选择名为 `Mindwtr` 的专用日历，日历应用可以使用该日历自身的颜色显示如水项目。
...
4. 在 Google Calendar Android 应用中，启用**与其他应用共享 Google Calendar 数据**，让 Android 向如水提供 Google 日历。
5. 返回如水的**设置 → 日历**，点按**刷新日历**，然后选择显示你的 Google 账户的 `Mindwtr` 目标。

如果由 Google 支持的 `Mindwtr` 日历尚未出现在如水中，说明 Android 还没有通过系统日历提供程序将其提供出来。刷新 Google Calendar，检查 Android 账户同步，在 Google Calendar 中启用**与其他应用共享 Google Calendar 数据**，然后在如水中点按**刷新日历**。
...
如果 `Mindwtr` 日历没有出现在目标列表中，请先确认它在 Apple Calendar 中可见，然后返回如水并点按**刷新日历**。

### 移动端：读取设备日历

在移动端，如水可以从设备日历数据库读取日历：

- **Android：**通过 Android 日历提供程序读取。如果某个同步应用没有通过该提供程序提供日历，如水就无法看到它们。
...
如水保持只读，不会针对日历源执行提供商 OAuth。

如水会从读取列表中隐藏自己推送的 `Mindwtr` 日历，以避免导入自身创建事件的重复副本。

### macOS：Apple Calendar 集成

在 macOS 桌面端，如水可以通过 EventKit 读取 Apple Calendar 事件，并推送已排期/到期的如水任务：

1. 打开**设置 -> 日历**
2. 请求 Apple Calendar 访问权限
3. 在 macOS 的**系统设置 -> 隐私与安全性 -> 日历**中允许如水访问
4. 如果希望将如水任务写入 Apple Calendar，请启用**将任务推送到日历**
...
事件会缓存在设备上，不会通过如水同步进行同步。

### ICS URL 要求

如水要求 URL 能获取原始 iCalendar 文本。可用的订阅源通常：
...
如果如水显示 `HTTP 401`，说明服务器正在请求身份验证。日历 URL 不支持用户名/密码提示、CalDAV 登录和 bearer token 标头。请改用提供商的秘密 iCalendar 订阅 URL。
...
- 桌面端和移动端的日历都可以从外部事件创建一个单独的如水任务。如可用，如水会复制事件标题、日期/时间、地点、描述和日历名称。
- 外部日历在如水中为**只读**。从事件创建任务不会修改原始事件。
- ICS 重复事件支持 `FREQ=DAILY`、`WEEKLY`、`MONTHLY` 和 `YEARLY`，并支持如水能够展开到可见范围内的模式所使用的 `INTERVAL`、`COUNT`、`UNTIL`、`BYDAY`、`BYMONTH` 和 `BYMONTHDAY`。
