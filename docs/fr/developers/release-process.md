...
- AUR `mindwtr-beta-bin` est compilé et validé lorsque les ressources de la préversion GitHub sont disponibles ; ses fichiers exacts `PKGBUILD` et `.SRCINFO` sont ensuite conservés comme artefact de révision. Le flux RC ne pousse rien vers AUR.
...
La bêta Flathub nécessite la branche bêta et les autorisations dans `flathub/tech.dongdongbh.mindwtr`. Les versions stables publient les paquets AUR `mindwtr-bin` et `mindwtr` depuis `release.yml` après une validation dans un conteneur propre et un contrôle de propriété. Le flux conserve l’arborescence exacte du paquet source comme artefact. Pour les builds RC, `mindwtr-beta-bin` reste une proposition ; utilisez le flux de récupération manuel `publish-aur.yml`, protégé par l’Environment `aur-publish`, lorsque l’AUR accepte les poussées. Une maintenance AUR peut retarder ce canal sans faire échouer la version Mindwtr.
...
| T-3 | Confirmez la préversion GitHub créée par `release-rc.yml`, vérifiez la pull request Flathub bêta et la proposition enregistrée pour `mindwtr-beta-bin` lorsque ces entrées étaient activées, puis annoncez la RC aux testeurs. |
