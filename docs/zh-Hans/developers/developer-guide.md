...
如水致力于做到**默认简洁，需要时强大**。我们专注于减轻认知负担、删繁就简，并让用户保持心流。每一项贡献都会根据以下原则进行权衡：
...
- ADR 0008 记录了如水目前有意继续采用基于快照的同步，而不添加增量日志。

贡献者应将快照传输视为一项有意为之的产品选择，而不是缺失的基础设施。只有当快照文件经常超过 5 MB、典型网络上的同步往返时间超过 5 秒，或如水需要实时多设备流式同步时，才应重新审视 ADR 0008。
