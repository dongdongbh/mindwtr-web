...
你可以在現有商店安裝上測試候選版本。請將商店中使用的 Microsoft 帳號信箱寄至 [support@mindwtr.app](mailto:support@mindwtr.app)，或在[測試版討論](https://github.com/dongdongbh/Mindwtr/discussions/769)中留言，我會將你加入 Mindwtr Beta。加入群組且測試套件發布後，在 **Microsoft Store → 媒體櫃**中檢查更新。請保留現有安裝，以保留任務與附件。加入和退出的說明請參閱[加入 Beta 管道](/zh-Hant/start/beta-channels)。
...
Windows 10 與 11 已內建 Winget。使用以下命令安裝 Mindwtr：
...
**在 Windows 上於可攜版與安裝版之間搬移。** 從 v1.3.2 起，安裝版在 `%APPDATA%\mindwtr` 下也使用同樣的 `config\` 和 `data\` 結構，所以可攜版的兩個子資料夾可以直接複製過去；WebView2 設定檔位於 `%LOCALAPPDATA%\tech.dongdongbh.mindwtr`。複製前請先關閉兩邊的 Mindwtr。

| 可攜版 | 安裝版 |
| --- | --- |
| `profile\data\`（`mindwtr.db`、`data.json`、`attachments\`、記錄、快照） | `%APPDATA%\mindwtr\data\` |
| `profile\config\config.toml` | `%APPDATA%\mindwtr\config\config.toml` |
| `profile\config\secrets.toml` | 沒有對應檔案：安裝版把密鑰存在 Windows 認證管理員中，搬移後需要重新輸入同步密語、Dropbox 登入或 API 金鑰 |
| `profile\webview\` | `%LOCALAPPDATA%\tech.dongdongbh.mindwtr\`（只是快取，可以不複製） |

仍須安裝 Windows WebView2。v1.1.0 以前的可攜式組建若曾將附件存於 `AppData\Roaming\mindwtr`，第一次啟動時會將附件移至可攜式設定檔；如果同一部電腦也安裝了 Mindwtr，附件則會改為複製，讓兩種安裝都能繼續運作。
...
[Mac App Store 上的 Mindwtr](https://apps.apple.com/app/mindwtr/id6758597144)
...
每個 GitHub 發布版本都包含一份 `SHA256SUMS` 校驗和清單以及覆蓋它的分離式 PGP 簽章 `SHA256SUMS.asc`。它們與 APT 和 RPM 儲存庫使用同一把金鑰簽署，金鑰發布於 `https://dongdongbh.github.io/Mindwtr/mindwtr.gpg.key`：

- 金鑰：Mindwtr Bot（RSA 4096）
- 指紋：`0358 999B BE70 4F58 8B90 9497 9E55 3245 CB17 047D`

同一金鑰也發佈在獨立於 GitHub 基礎設施的 `keys.openpgp.org` 金鑰伺服器上——請從兩個來源分別取得，並確認兩者顯示的指紋與上文一致。
...
`gpg --verify` 必須回報來自上述指紋的有效簽章。在 macOS 上，校驗和步驟請使用 `shasum -a 256 --check --ignore-missing SHA256SUMS`。
...
v1.3.2 之前，Windows 和 macOS 把這些檔案都直接放在 `mindwtr` 資料夾裡。v1.3.2 首次啟動時會把它們移到 `config/` 和 `data/` 中，資料夾裡的其他內容不會被更動。之後不支援退回舊版本：舊版本只會在原來的位置尋找。

---

## 更新

更新檢查會辨識發行管道：Mindwtr 會偵測安裝方式、比對該管道發布的版本，再引導你採用該管道的更新方式。
...
- **Scoop**：Mindwtr 完全不會自動檢查更新，因為任何 bucket 都可能提供 manifest，且更新由 Scoop 負責（`scoop update mindwtr`）。仍可在「設定 → 關於」中手動檢查。
- **Flatpak、Snap 與應用程式商店的自動更新**：更新會自動送達，因此 Mindwtr 不會顯示提醒。
