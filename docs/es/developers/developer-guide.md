...
Para entornos de compilación mínimos (contenedores, Nix), el conjunto completo de dependencias nativas más allá de la pila WebKitGTK/GTK3 es: `cmake` y una cadena de herramientas C/C++ compatible con `bindgen` (transcripción local con Whisper), `perl` (la compilación de OpenSSL), las cabeceras de ALSA (audio) y `libayatana-appindicator` (icono de la bandeja del sistema).

En [TheRealGramdalf/mindwtr-flake](https://github.com/TheRealGramdalf/mindwtr-flake) hay disponible un flake de Nix mantenido por la comunidad que empaqueta la aplicación de escritorio, el servidor de sincronización y la aplicación web. Se mantiene fuera de este repositorio y no forma parte del proceso oficial de publicación.
...
│           ├── i18n/      # Cargador de traducciones y configuraciones regionales
│           └── ...
│
...
El [árbol `mindwtr-web/docs`](https://github.com/dongdongbh/mindwtr-web/tree/main/docs) es la fuente canónica de la documentación pública para usuarios y desarrolladores y se publica en https://docs.mindwtr.app/. El directorio `docs/` del repositorio de la aplicación contiene artefactos de ingeniería y publicación, como ADR, guías de contribución y procesos, y notas de versión.
...
| **Enrutador**  | Estado interno de vistas | Expo Router      | N/A              |
...
Si no se proporciona ninguno de los dos, el asistente sale al arrancar. Define `MINDWTR_API_CORS_ORIGIN` con un origen http(s) exacto si una página del navegador debe llamarla; sin ese valor no se envían cabeceras CORS.
...
Sustituye el marcador del token local en el comando de inicio y en las solicitudes por el mismo token seguro. Sustituye `<id>` por el ID devuelto al crear la tarea.
...
Sustituye el marcador del token de Cloud por un token seguro y usa ese token en tus clientes.
