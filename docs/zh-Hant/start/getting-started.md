...
全新安裝時，如水會詢問你要從頭開始、匯入備份，或連接同步。設定完成後，預設開啟**專注**檢視，讓今日行事曆項目與下一步行動優先呈現。需要迅速記下一個想法時，隨時將新工作收進**收集箱**。
...
如水支援自然語言快速新增，直接在任務輸入欄中輸入即可。完成專案最後一項行動時出現的「下一步行動是什麼？」提示也支援相同語法（例如以 `/waiting` 結尾，將後續追蹤設為等待中）：
...
- 瀏覽[常見問題](/zh-Hant/start/faq)，或閱讀完整的[如水 GTD 工作流程](/zh-Hant/use/gtd-workflow)指南。
