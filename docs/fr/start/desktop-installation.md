...
Un paquet compilé depuis les sources, `mindwtr`, existe également sur l’AUR. Il est maintenu par la communauté, pas par le projet Mindwtr, et il ne fait pas partie du processus de publication de Mindwtr. Utilisez `mindwtr-bin`, sauf si vous devez compiler localement.

#### Vérifier les paquets AUR officiels

Mindwtr publie deux identités AUR actuelles : `mindwtr-bin` (binaire stable) et [`mindwtr-beta-bin`](/fr/start/beta-channels) (préversion). Un troisième nom, `mindwtr-bin-beta`, correspond à l'ancienne identité bêta ; il continue d'être mis à jour à partir des mêmes artefacts de publication signés pendant le changement de nom. La source du paquet doit pointer vers `github.com/dongdongbh/Mindwtr`, et le mainteneur doit être `dongdongbh`. Vérifiez chaque modification de `PKGBUILD` et `.SRCINFO` avant la compilation. Les nouveaux manifestes de sommes de contrôle sont signés avec la clé `0358 999B BE70 4F58 8B90 9497 9E55 3245 CB17 047D`.
...
### Bêta Microsoft Store

Vous pouvez tester les versions candidates avec la même installation du Store. Envoyez l’adresse de votre compte Microsoft du Store à [support@mindwtr.app](mailto:support@mindwtr.app), ou partagez-la dans la [discussion bêta](https://github.com/dongdongbh/Mindwtr/discussions/769), et je vous ajouterai à Mindwtr Beta. Après l’inscription et la publication du flight, recherchez les mises à jour dans **Microsoft Store → Bibliothèque**. Conservez votre installation pour garder vos tâches et pièces jointes. Consultez [Rejoindre les canaux bêta](/fr/start/beta-channels) pour les modalités d’inscription et de départ.
...
[![Version Scoop](https://img.shields.io/scoop/v/mindwtr?bucket=extras&label=Scoop&logo=scoop&logoColor=white&color=E6E6E6)](https://github.com/ScoopInstaller/Extras/blob/master/bucket/mindwtr.json)
...
**Passer de la version portable à la version installée sous Windows, et inversement.** Depuis la v1.3.2, l’app installée utilise la même organisation `config\` et `data\` sous `%APPDATA%\mindwtr`, si bien que les deux sous-dossiers portables se copient tels quels ; le profil WebView2 est dans `%LOCALAPPDATA%\tech.dongdongbh.mindwtr`. Fermez Mindwtr des deux côtés avant de copier.

| Portable | Installée |
| --- | --- |
| `profile\data\` (`mindwtr.db`, `data.json`, `attachments\`, journaux, instantanés) | `%APPDATA%\mindwtr\data\` |
| `profile\config\config.toml` | `%APPDATA%\mindwtr\config\config.toml` |
| `profile\config\secrets.toml` | pas un fichier : l’app installée range les secrets dans le Gestionnaire d’identification Windows ; ressaisissez la phrase secrète de synchronisation, la connexion Dropbox ou les clés API après le déplacement |
| `profile\webview\` | `%LOCALAPPDATA%\tech.dongdongbh.mindwtr\` (cache seulement ; peut être ignoré) |
...
Chaque version publiée sur GitHub inclut un manifeste de sommes de contrôle `SHA256SUMS` et une signature PGP détachée `SHA256SUMS.asc` qui le couvre. Ils sont signés avec la même clé que les dépôts APT et RPM, publiée sur `https://dongdongbh.github.io/Mindwtr/mindwtr.gpg.key` :
...
`gpg --verify` doit signaler une signature valide avec l'empreinte ci-dessus. Sous macOS, utilisez `shasum -a 256 --check --ignore-missing SHA256SUMS` pour l'étape de la somme de contrôle.
...
Avant la v1.3.2, Windows et macOS gardaient tous ces fichiers directement dans le dossier `mindwtr`. Le premier démarrage de la v1.3.2 les déplace dans `config/` et `data/` et ne touche à rien d’autre dans le dossier. Revenir ensuite à une version antérieure n’est pas pris en charge : elle ne regarde qu’à l’ancien emplacement.
