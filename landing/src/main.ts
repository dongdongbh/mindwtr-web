...
function applyPlatform(platform: Platform | null): void {
  ...
  // Unrecognized and genuine Linux visitors stay on the complete download
  // page, where every platform remains available.
...
function init(): void {
  applyPlatform(detectPlatform(navigator));
  wireCopyButtons();
  wireLanguageSwitch();
  wireHeaderMorph();
  initCoverflow();
