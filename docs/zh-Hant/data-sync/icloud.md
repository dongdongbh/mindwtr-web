...
在原生模組可用的 Apple 裝置上，Mindwtr 支援原生 **iCloud / CloudKit** 同步後端。
...
2. 確認這些裝置已為 Mindwtr 啟用 iCloud。
3. 在 Apple 裝置的 Mindwtr 中開啟**設定 → 同步**。
4. 選擇 **iCloud** 作為同步後端。
5. 執行一次同步，以寫入初始資料或拉取既有資料。

設定完成後，Mindwtr 會繼續採用一般的在地優先合併流程，並在可用時回應 CloudKit 變更通知。

## 平台注意事項

- 若非 Apple 版本讀到舊的 `cloudkit` 後端值，Mindwtr 會改用「關閉」，而不是顯示無法使用的 iCloud 選項。
