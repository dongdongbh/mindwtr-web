...
如水通过 iPhone 和 iPad 上的原生 App Intents 支持 Apple 快捷指令。首个版本聚焦 GTD 记录循环：快速把未闭合事项放入如水，再在应用内回顾和处理。

这套功能有意小于 Things 已成熟的快捷指令系统。Things 提供创建、查找、编辑、显示和自定义项目/清单操作。如水 v1 从记录与导航开始，确保可靠，并且不会绕过如水常规的任务创建、修订和同步路径。

## 可用范围

包含如水 App Intents 集成的 iOS 构建可使用 Apple 快捷指令。
...
| 任务 | 是 | 任务标题。空标题会被拒绝。 |
| 备注 | 否 | 添加为任务描述。 |
| 标签 | 否 | 逗号分隔的标签。如水保存时会规范化为 `#tag`。 |
| 项目 | 否 | 按标题匹配活跃项目；保存记录时也可创建项目。 |

运行时会：

1. 快捷指令打开如水。
2. 如水显示记录界面，并填入标题和可选元数据。
3. 你检查记录，再通过如水常规流程保存。

Swift 不会直接写入任务，因此任务创建始终通过如水现有的存储、SQLite、修订和同步逻辑。
...
使用**添加到如水收集箱**可在不打开如水的情况下静默创建任务。这项操作适合用于快捷指令自动化。时间、日历或位置触发器无需有人操作手机即可添加任务。
...
| 备注 | 否 | 添加为任务描述。 |
| 标签 | 否 | 逗号分隔的标签。如水会规范化为 `#tag`。 |
| 项目 | 否 | 按标题匹配活跃项目。未知或已归档项目会被忽略，任务仍会进入收集箱。 |

运行时会：

1. 操作在设备上将记录加入队列并立即结束。如水保持在后台。
2. 下次打开如水（或返回前台）时，排队的任务会通过常规存储与同步路径创建到收集箱。

由于任务在下次打开应用时才创建，因此在这台 iPhone 或 iPad 上再次运行如水之前，它不会出现在其他已同步设备上，也不会触发提醒。与**记录到如水**不同，这项操作绝不会创建新项目。

### 示例：日历触发的任务

1. 在**快捷指令**应用中打开**自动化**并新建自动化。
2. 选择触发器，例如标题包含“garbage collection”的日历事件。
3. 添加如水的**添加到如水收集箱**操作，将**任务**设为“Take out the trash”。
4. 将自动化设为**立即运行**，无需确认。
...
如水还支持 URL scheme 自动化。其他自动化工具无法发现原生 App Intents 时可使用。
...
这些都是很好的未来候选功能，但需要谨慎设计，因为编辑和后台写入必须保持如水的本地优先同步与 GTD 工作流程规则。
