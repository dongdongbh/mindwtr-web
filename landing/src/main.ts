...
function applyPlatform(platform: Platform | null): void {
  ...
  // complete download page instead of auto-redirecting an actual Linux user.
  // On a touch-first device, make Google Play one tap away at the top while
  // retaining the full Linux and cross-platform choices below.
  if (directDownload) {
    const touchFirst = navigator.maxTouchPoints > 0
      || (typeof window.matchMedia === "function" && window.matchMedia("(pointer: coarse)").matches);
    if (platform === "linux" && touchFirst && cta instanceof HTMLAnchorElement) {
      cta.textContent = JS_STRINGS[pageLocale()].downloadFor(PLATFORM_LABEL.android);
      cta.setAttribute("href", DIRECT_STORE_URL.android ?? "#download-android");
    }
    return;
  }
 ...
 * stops while its always-visible controls remain available. If the preference
 * changes again, restore autoplay only for clips that originally requested it.
 */
function calmShotVideos(): void {
  const motionPreference = window.matchMedia("(prefers-reduced-motion: reduce)");
  const videos = Array.from(
    document.querySelectorAll<HTMLVideoElement>(".article-shot video, .feature-shot video"),
    (video) => ({ video, shouldAutoplay: video.hasAttribute("autoplay") }),
  );
  const applyMotionPreference = (): void => {
    videos.forEach(({ video, shouldAutoplay }) => {
      if (!shouldAutoplay) return;
      video.autoplay = !motionPreference.matches;
      video.toggleAttribute("autoplay", !motionPreference.matches);
