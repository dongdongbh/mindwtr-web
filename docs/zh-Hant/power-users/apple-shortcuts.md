...
iOS 小工具無法顯示文字欄位或收集視窗：小工具按鈕只能開啟 App，或執行沒有畫面的動作。想要類似 Android 的流程，也就是點一下就在目前 App 上方跳出小提示框，只需建立一次這個捷徑：

1. 在**捷徑**中建立新捷徑，命名為「收集」。
2. 加入**要求輸入**（類型為文字，提示為「任務」）。在提示框中點麥克風即可改用語音聽寫。
3. 加入 Mindwtr 的 **Add to Mindwtr** 動作，並把輸入傳給 **Task**。這裡支援快速新增語法（`/due:tomorrow`、`@phone`、`#tag`）。
4. 把它放在小工具會在的位置：在捷徑詳細資訊中選**加入主畫面**，就會在小工具旁多一個圖示；**設定 → 動作按鈕**可用側邊按鈕執行；iOS 18 可加入控制中心；**設定 → 輔助使用 → 觸控 → 背面輕點**可用雙擊手機背面執行。

執行時會跳出小文字提示框，儲存後回到你原本在做的事。Mindwtr 維持關閉，任務會在下次開啟 App 時建立到收集箱，和所有 **Add to Mindwtr** 收集一樣。若想在儲存前先檢查任務，請改用 **Capture to Mindwtr**。
