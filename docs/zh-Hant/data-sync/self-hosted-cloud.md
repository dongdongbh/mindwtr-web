...
自架伺服器可以發布一個唯讀的 iCalendar 訂閱來源，包含你已排程的任務與截止日期，任何行事曆應用程式都能訂閱。

1. 在 Mindwtr 桌面版開啟設定，進入同步，並選擇自架。
2. 在「行事曆訂閱」下選擇 **產生網址**。
3. 複製該網址，並在行事曆應用程式中加入為訂閱行事曆。

訂閱來源顯示的內容與行事曆檢視一致：已排程的任務顯示在其開始時間，截止日期顯示為當天的全天事件；已完成、已封存、參考資料、已刪除以及不在使用中專案的任務都不會出現。事件僅包含任務標題，描述、清單、附件、標籤與專案資訊永遠不會發布。

::: warning 任何取得此網址的人都能讀取訂閱內容
此網址是唯一的憑證，上面沒有密碼。請像分享私密連結那樣分享它；選擇 **重新產生** 可讓舊網址失效，選擇 **撤銷** 可完全下線此訂閱來源。
:::

訂閱來源是唯讀的。在行事曆應用程式中編輯事件不會改變 Mindwtr 中的任務；它也不是 CalDAV——此訂閱來源是單向的。
