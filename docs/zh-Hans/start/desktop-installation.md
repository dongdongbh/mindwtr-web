...
[AppImage 应用目录](https://appimage.github.io/Mindwtr/)
...
Windows 10 和 11 已内置 Winget。使用以下命令安装 Mindwtr：
...
2. 打开磁盘映像
3. 将 Mindwtr 拖到 Applications 文件夹
...
更新检查会识别安装渠道：Mindwtr 会检测其安装方式，与该渠道发布的版本进行比较，并将你引导至该渠道的更新方式。
...
- **Scoop**：Mindwtr 完全不会自动检查更新，因为任何 bucket 都可以提供清单，且更新由 Scoop 管理（`scoop update mindwtr`）。“设置 → 关于”中的手动检查仍然有效。
- **Flatpak、Snap 和应用商店的自动更新**：更新会自动送达，因此 Mindwtr 不会显示提醒。
...
### macOS
将 Mindwtr 从 Applications 拖到废纸篓。
