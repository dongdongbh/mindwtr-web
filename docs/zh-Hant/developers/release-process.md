...
Flathub beta 需要 `flathub/tech.dongdongbh.mindwtr` 中的 beta 分支與權限。AUR 發布使用獨立的手動工作流程，並受 GitHub `aur-publish` Environment 保護。請審核儲存的提案、核准 Environment 部署，且僅在 AUR 恢復推送後發布。AUR 上游維護只會延遲該管道，不會使 Mindwtr 發布失敗。
