...
- `contentRev` et `fileHash` constituent l’identité de contenu synchronisée du fichier joint. Un appareil n’incrémente `contentRev` qu’après qu’une empreinte a confirmé que les octets ont réellement changé ; un `contentRev` plus élevé emporte donc les champs de contenu, même lorsque l’autre côté gagne la fusion au niveau de la pièce jointe, et une égalité s’en remet à ce gagnant. Un `contentRev` absent vaut `0`, de sorte que les clients antérieurs au suivi de contenu se fusionnent proprement.
- La date de modification et la taille enregistrées du fichier restent locales à l’appareil et sont retirées avant toute écriture distante. Elles décrivent la copie des octets propre à cet appareil, et chaque appareil les redérive après son propre envoi ou téléchargement. C’est précisément parce qu’elles ne figurent pas dans l’enregistrement synchronisé qu’un cycle de synchronisation peut repérer un fichier modifié localement par comparaison, sans écrire une modification distante à chaque cycle.
