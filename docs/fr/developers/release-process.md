...
- AUR `mindwtr-bin-beta` est compilé et validé lorsque les ressources de la préversion GitHub sont disponibles ; ses fichiers exacts `PKGBUILD` et `.SRCINFO` sont ensuite conservés comme artefact de révision. Le flux RC ne pousse rien vers AUR.
...
| T-3 | Confirmez la préversion GitHub créée par `release-rc.yml`, vérifiez la pull request Flathub bêta et la proposition enregistrée pour `mindwtr-bin-beta` lorsque ces entrées étaient activées, puis annoncez la RC aux testeurs. |
