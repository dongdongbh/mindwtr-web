...
如水的設計是**預設簡單，需要時強大**。我們著重降低認知負擔、去除贅餘，並讓使用者保持專注。每項貢獻都會依下列原則衡量：
...
- ADR 0008 記錄如水目前刻意維持快照式同步，而不加入差異日誌。

貢獻者應將快照傳輸視為刻意的產品選擇，而非缺少的基礎設施。只有在快照檔案經常超過 5 MB、一般網路的同步往返時間超過 5 秒，或如水需要即時多裝置串流時，才應重新評估 ADR 0008。
