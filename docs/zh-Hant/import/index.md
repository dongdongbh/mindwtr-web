...
本指南說明如何將其他任務管理工具中的任務移入如水。這適用於完整遷移，而非快速收集單一項目。

## 匯入前的準備

遷移正是拋開陳年雜務的好時機。若先前的應用程式中有數百項過時任務，建議只移入目前的專案、仍在進行的下一步行動，以及可信賴的參考項目。

如水可以匯入受支援應用程式的完整匯出檔，但經過篩選、規模較小的遷移，通常比把所有舊任務複製到新系統更符合 GTD。

## 可用的匯入工具

對於匯出格式結構清楚、能安全對應資料的少數應用程式，如水提供完整支援的匯入工具：
...
開啟**設定 → 資料**，選擇相應的匯入動作。如水會在新增任何內容前顯示預覽。
...
每個非空白行都會成為一項任務。每一行都會依如水的快速新增語法解析，因此可直接在行內加入中繼資料：
...
若原本的應用程式可匯出 `.txt` 檔案，請從如水的收集介面使用文字匯入功能。
...
每行放一項任務。若希望如水在匯入時設定中繼資料，請在同一行加入快速新增符記：
...
如水使用 `/note:` 等明確的快速新增命令，而不是隱藏的 Tab 字元筆記格式。這讓文字檔在匯入前仍容易閱讀，並採用與一般收集相同的解析器。

### 將 CSV 轉成快速新增文字的指令碼

若應用程式可匯出 CSV，但如水沒有相應的原生匯入工具，可將 CSV 轉成純文字快速新增檔，再匯入該 `.txt` 檔案。
...
技術使用者可以針對如水的自動化介面編寫自己的匯入工具：

- [本機 API](/zh-Hant/power-users/local-api)
- [MCP 伺服器](/zh-Hant/power-users/mcp)
- 在儲存庫簽出版本中執行 `bun run mindwtr:cli -- --help`

若原本的應用程式可匯出結構化 JSON 或 CSV，而你需要比純文字更大的控制彈性，請使用這種方式。這些工具會經由如水的一般資料模型運作，但自訂遷移指令碼需由你自行維護。

## 若清單中沒有你的應用程式

請依下列順序處理：

1. 確認應用程式能否匯出為如水已支援匯入的格式。
...
原生匯入工具會依需求量，以及來源格式能否清楚對應至如水的 GTD 模型來排定優先順序。
