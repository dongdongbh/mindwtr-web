...
Mindwtr 致力于做到**默认简洁，需要时强大，生活忙乱时也宽容**。我们专注于减轻认知负担、删繁就简，并让用户保持心流。每一项贡献都会根据以下原则进行权衡：
...
- **生活忙乱时也宽容**：绝不因为用户当时力不从心而让他们内疚。离开一段时间后回来必须轻松，大量积压交给一次有边界、做得完的处理，长期没动的事项是重新考虑的邀请，并且没有连续打卡，也没有效率评分。
...
社区维护的 Nix flake（打包桌面应用、同步服务器和 Web 应用）可在 [TheRealGramdalf/mindwtr-flake](https://github.com/TheRealGramdalf/mindwtr-flake) 获取。它在本仓库之外维护，不属于官方发布流程。
...
两者都未提供时，辅助程序会在启动阶段退出。如果需要浏览器页面调用它，请把 `MINDWTR_API_CORS_ORIGIN` 设为一个精确的 http(s) 来源；未设置时不会发送 CORS 响应头。
