...
> API 金鑰、自訂 AI 端點 URL（包括語音轉文字端點）、額外的請求參數與本機模型路徑永遠不會同步。自訂端點需要在每台裝置上分別填寫一次。
...
在 Android 上，請為檔案同步選取裝置上的本機資料夾。若要使用 Google Drive，請透過獨立的資料夾同步應用程式將雲端資料夾鏡像到該本機資料夾，然後在 Mindwtr 中選取這個本機資料夾；請勿直接選取 Google Drive 雲端位置。請參閱上文的 **Android 上的 Google Drive（檔案同步）與 Dropbox 檔案同步備援**。Dropbox 或 WebDAV 是可直接連線的替代方案。

### 1. 先匯出資料

1. 前往**設定 → 資料**
2. 點選**匯出備份**
3. 將檔案儲存至同步資料夾。在 Android 上，請使用之後將在 Mindwtr 中選取的同一個裝置本機資料夾
...
- **自動匯入**：選定清單後，此開關會在每次開啟 Mindwtr 或回到前景時執行同樣的匯入，讓隨手記下的提醒事項不必手動點按就進入收集箱。iOS 不允許 App 在背景讀取提醒事項，因此只在 App 開啟時檢查。匯入了內容時會顯示簡短提示。
...
- 設有 heartbeat 分析的組建可能會傳送一項小型應用程式健康事件；其中不包含任務、專案、筆記、檔案、AI 提示或帳號內容。它會附上一個隨機的安裝識別碼和一個隨機的個人資料識別碼。個人資料識別碼儲存在你的同步資料中，因此你的多部裝置會被計為一個個人資料，而不是多個人。請參閱[隱私權政策](https://mindwtr.app/privacy)。
- 你完全掌控自己的資料
