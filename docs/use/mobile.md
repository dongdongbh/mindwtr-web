...
2. Select **Widgets**
3. Find **Mindwtr** and add either the **Tasks** widget or the one-cell **Quick capture** button. The Tasks widget shows a **+** button and your tasks, each with its priority colour, its project or area, and its due date. On Android, a newly added Tasks widget shows Focus, with the inbox count in its header and the same sections as the Focus screen: Today's Focus, Today, Review Due, Next actions, Upcoming, in the default Focus order and with no filters. To switch the list, tap the widget's title, which carries a small down arrow. A sheet lists Focus, Inbox, Next Actions, Waiting For, Someday/Maybe and your projects, whose sections become headers. Tap one and the widget redraws. This works on every launcher, and some launchers have no edit action of their own for widgets. You can add several Tasks widgets with different lists.
4. Tap a task to open it, tap the ring at the start of a row to complete it, or tap **+** to capture

On Android, tapping the ring completes the task. The row shows struck through straight away, and a second tap within about three seconds undoes it. The task stays struck through on the widget until Mindwtr next runs and completes it there, so it also syncs to your other devices from the app.
