...
- À partir de la prochaine version, l’export filtré sur ordinateur se trouve dans **Plus (⋯) → Export current results as CSV**, à côté du nom de l’application dans la barre latérale. Il est disponible dans les listes de tâches, Contextes, les recherches enregistrées, un projet, Focus, Tableau et Révision. Il exporte les résultats après les filtres, la recherche et la sélection de domaine, y compris les groupes repliés et les lignes hors écran. L’action est désactivée sans résultats ou dans une vue non prise en charge. L’export complet reste dans les paramètres.
