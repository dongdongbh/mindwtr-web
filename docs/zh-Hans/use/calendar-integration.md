...
- **周视图**：用于浏览当前工作周的规划网格，默认显示七天。
- **月视图**：通过标记概览截止日期、已排期任务和事件。
- **日程视图**：未来 60 天的滚动列表，显示即将到来的截止日期、已排期任务和外部事件。

周视图可显示两到七天。桌面端可用标题栏中的控件调整天数，移动端可在时间线上上下滑动调整。减少天数后，其余各列会变宽，便于阅读较长的标题，也便于将任务拖放到具体时间。可见日期始终位于同一周内并包含当前日期；因此，从周一开始的五日视图在切换周次时仍会显示周一至周五。Mindwtr 会按设备保存这项布局选择，不会同步。

### 回看已完成的工作

日历默认只显示尚未完成的工作。日历标题栏中的 **已完成** 开关还会显示已完成和已归档的任务，按它们完成的那一天摆放，而不是按它们可能仍带有的开始日期或截止日期。它们以灰显加删除线呈现，因此绝不会被误看成待办工作，也无法拖动到别的日期——完成是已发生之事的记录，不是计划。
