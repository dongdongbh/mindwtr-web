...
- [匯入 Apple 提醒事項](/zh-Hant/data-sync/#apple-提醒事項匯入-ios) - 僅限 iOS，可從所選的「提醒事項」清單匯入未完成的提醒事項，可手動匯入或在每次開啟 App 時自動匯入
...
| [Mindwtr CSV](/zh-Hant/import/mindwtr-csv) | 採用文件所列欄位名稱的 CSV 檔案，或內含多個 CSV 的 ZIP | 領域、專案、分區、狀態、情境、標籤、負責人、優先順序、精力、開始日期、截止日期、複查日期、完成時間、檢查清單、地點、手動排序與受支援的重複規則 | Mindwtr 無法表達並連同所在列提示的重複規則、無法解析的日期、重複的 ID，以及因缺少標題而略過的列 |
...
4. 如果對應不正確，請在結果提示中點按**復原匯入**，或稍後從**設定 → 同步 → 復原快照**還原，調整來源匯出檔後重試。部分匯入工具重複匯入同一個檔案時可能產生重複任務。
...
如果你已經在使用 AI 代理(Claude Desktop、Claude Code、Codex、Gemini CLI),[MCP 伺服器](/zh-Hant/power-users/mcp)就是最靈活的匯入器。把代理連線到 Mindwtr,將舊應用程式的資料以任何形式交給它(CSV、JSON 匯出,甚至應用程式原始的 SQLite 資料庫檔案),再描述你想要的遷移方式:

```text
這是我舊應用程式的 SQLite 資料庫。請讀取其中的專案與任務,並在 Mindwtr
中重建:清單作為專案,保留截止日期與備註,略過已完成的任務。
```

代理會自行辨識來源格式,把欄位對應到 Mindwtr 的資料模型,並透過與一般擷取相同的工具建立領域、專案與任務。不需要轉換指令碼,也不需要重新命名欄位:即使應用程式沒有匯入器、也沒有乾淨的匯出,這條路徑同樣適用。

三點實用提示:

- 先做好[備份](/zh-Hant/data-sync/backup-restore)。MCP 寫入遵循 Mindwtr 的正常資料模型,但這條路徑沒有匯入預覽,也沒有一鍵復原;若要還原,請使用快照。
- 先遷移一個小專案並檢查結果,再交出完整資料庫。
- 匯出內容會經過你使用的 AI 模型;在交付私人任務之前,請確認你能接受服務商的資料政策。
