...
- AUR `mindwtr-beta-bin` se compila y valida cuando existen los recursos del prelanzamiento de GitHub; después, sus archivos exactos `PKGBUILD` y `.SRCINFO` se guardan como artefacto para revisión. El flujo de RC no envía cambios a AUR.
...
| T-3 | Confirma el prelanzamiento de GitHub de `release-rc.yml`, verifica la solicitud de incorporación de cambios de la beta de Flathub y la propuesta guardada de `mindwtr-beta-bin` cuando esas entradas estuvieran habilitadas, y anuncia la RC a quienes vayan a probarla. |
