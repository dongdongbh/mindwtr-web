...
- **週檢視：**用於瀏覽目前工作週的規劃格線，預設顯示七天。
- **月檢視：**以標記呈現截止期限、已排程任務與事件的概覽。
- **行程檢視：**未來 60 天的截止期限、已排程任務與外部事件清單。

週檢視可顯示兩到七天。桌面版可使用標題列中的控制項調整天數，行動版可在時間軸上向上或向下滑動調整。減少天數後，其餘各欄會變寬，方便閱讀較長的標題，也方便將任務拖放到特定時間。顯示的日期會維持在同一週內並包含目前日期；因此，從週一開始的五日檢視在切換週次時仍會顯示週一至週五。Mindwtr 會依裝置保存這項版面配置選擇，不會同步。

### 回顧已完成的工作

行事曆預設只顯示尚未完成的工作。行事曆標題列中的 **已完成** 開關還會顯示已完成與已封存的任務，依它們完成的那一天擺放，而不是依它們可能仍帶有的開始日期或截止日期。它們以淡化加刪除線呈現，因此絕不會被誤看成待辦工作，也無法拖曳到其他日期——完成是已發生之事的紀錄，不是計畫。
