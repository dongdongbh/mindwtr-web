...
- Ab der nächsten Version finden Sie den CSV-Export auf dem Desktop bei den Aktionen für die Mehrfachauswahl. Filtern Sie eine Aufgabenliste oder die Kontextansicht, klicken Sie auf **Auswählen**, markieren Sie die gewünschten Aufgaben und wählen Sie **Ausgewählte Aufgaben als CSV exportieren**. Nur markierte Aufgaben werden exportiert, auch außerhalb des sichtbaren Ausschnitts. Für die gesamte gefilterte Liste klappen Sie zuerst die gewünschten Gruppen auf und wählen dann alle Aufgaben aus. Der Export verändert weder Aufgaben noch Auswahl. Der vollständige Export bleibt in den Einstellungen.
