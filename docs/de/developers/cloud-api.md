...
| `isFocusedToday` | `true`/`1` liefert markierte Aufgaben, einschließlich nächster Aktionen mit zukünftigem Start, die für den Fokus vorgemerkt sind; `false`/`0` liefert die übrigen. Weglassen bedeutet kein Filter. |
...
`assignedTo` ist der Name einer einzelnen Person als Zeichenfolge, zum Beispiel `"Alex"`, kein Array von Personen-IDs. Mit `null` oder `""` wird die Zuweisung entfernt. Beim Erstellen und Aktualisieren von Aufgaben über REST werden Arrays, Objekte, Zahlen und boolesche Werte mit HTTP 400 abgelehnt. Die Suche ignoriert bereits gespeicherte Zuweisungen mit ungültigem Datentyp, ohne sie umzuschreiben.
