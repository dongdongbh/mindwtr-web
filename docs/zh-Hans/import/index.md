...
| [Mindwtr CSV](/zh-Hans/import/mindwtr-csv) | 使用文档所列列名的 CSV 文件，或包含多个 CSV 的 ZIP | 领域、项目、分区、状态、情境、标签、负责人、优先级、精力、开始日期、截止日期、复查日期、完成时间、检查清单、地点、手动排序和受支持的重复规则 | Mindwtr 无法表达并连同所在行提示的重复规则、无法解析的日期、重复的 ID，以及因没有标题而跳过的行 |
...
4. 如果映射不正确，请在结果提示中点按**撤销导入**，或稍后从**设置 → 同步 → 恢复快照**恢复，调整源导出后重试。部分导入器重复导入同一个文件时可能产生重复任务。
...
如果你已经在使用 AI 代理(Claude Desktop、Claude Code、Codex、Gemini CLI),那么 [MCP 服务器](/zh-Hans/power-users/mcp)就是最灵活的导入器。把代理连接到 Mindwtr,将旧应用的数据以任意形式交给它(CSV、JSON 导出,甚至应用原始的 SQLite 数据库文件),再描述你想要的迁移方式:

```text
这是我旧应用的 SQLite 数据库。请读取其中的项目和任务,并在 Mindwtr 中
重建:列表作为项目,保留截止日期和备注,跳过已完成的任务。
```

代理会自行识别源格式,把字段映射到 Mindwtr 的数据模型,并通过与普通捕获相同的工具创建领域、项目和任务。不需要转换脚本,也不需要重命名列:即使应用没有导入器、也没有规范的导出,这条路径同样适用。

三点实用提示:

- 先做好[备份](/zh-Hans/data-sync/backup-restore)。MCP 写入遵循 Mindwtr 的正常数据模型,但这条路径没有导入预览,也没有一键撤销;恢复需要还原快照。
- 先迁移一个小项目并检查结果,再交出完整数据库。
- 导出内容会经过你所使用的 AI 模型;在提交私人任务之前,请确认你能接受服务商的数据政策。
