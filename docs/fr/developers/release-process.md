...
La bêta Flathub nécessite la branche bêta et les autorisations dans `flathub/tech.dongdongbh.mindwtr`. La publication AUR utilise un flux manuel distinct protégé par l’Environment GitHub `aur-publish`. Examinez la proposition enregistrée, approuvez le déploiement de l’Environment et ne publiez que lorsque l’AUR accepte les poussées. Une maintenance AUR retarde ce canal sans faire échouer la version Mindwtr.
