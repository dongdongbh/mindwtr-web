...
- [Importar Recordatorios de Apple](/es/data-sync/#importacion-de-recordatorios-de-apple-ios): importación exclusiva de iOS para recordatorios incompletos de una lista seleccionada., manual o automáticamente cada vez que se abre la app
- [Importar desde CSV de Mindwtr](/es/import/mindwtr-csv): un formato de columnas documentado para cualquier aplicación que no tenga importador propio.

Abre **Ajustes → Datos** y elige la acción de importación correspondiente: Todoist, TickTick, DGT GTD, OmniFocus o el CSV genérico de Mindwtr. Mindwtr muestra una vista previa antes de añadir nada.
...
Esta cobertura se revisó el 9 de agosto de 2026 con el código de importación del commit [e787c2e](https://github.com/dongdongbh/Mindwtr/commit/e787c2e8cbe1ac93330a55d89fe5edc677f0eefe) de Mindwtr. Los datos de prueba cubren CSV y ZIP de Todoist, CSV y ZIP de TickTick 7.1, JSON y ZIP del esquema 3 de DGT, CSV, CSV UTF-16, JSON y ZIP de OmniFocus, y el CSV de Mindwtr como archivo suelto y dentro de un ZIP. Los formatos de exportación pueden cambiar. Revisa la vista previa y la guía específica antes de confirmar.
...
| [CSV de Mindwtr](/es/import/mindwtr-csv) | Un archivo CSV, o un ZIP de archivos CSV, con los nombres de columna documentados | Áreas, proyectos, secciones, estados, contextos, etiquetas, personas asignadas, prioridad, energía, fechas de inicio, límite, revisión y finalización, listas de comprobación, ubicación, orden manual y recurrencias admitidas | Las reglas de repetición que Mindwtr no puede expresar, que se informan junto con su fila; fechas ilegibles; identificadores repetidos; y filas omitidas por no tener título |
...
Este script es un punto de partida, no un importador específico compatible oficialmente. No conservará tareas anidadas, adjuntos, recurrencias ni campos propios de una aplicación a menos que lo adaptes. La [importación nativa desde CSV de Mindwtr](/es/import/mindwtr-csv) ya cubre casi todo aquello para lo que se escribió el script, y además conserva secciones, estados, listas de comprobación y áreas que el texto de adición rápida no puede expresar, así que recurre al script solo cuando necesites exactamente su salida.
...
### Migración asistida por IA (MCP)

Si ya usas un agente de IA (Claude Desktop, Claude Code, Codex, Gemini CLI), el [Servidor MCP](/es/power-users/mcp) lo convierte en el importador más flexible de todos. Conecta el agente a Mindwtr, entrégale los datos de tu antigua aplicación en la forma en que existan (un CSV, una exportación JSON o incluso el archivo de base de datos SQLite de la aplicación) y describe la migración que quieres:

```text
Aquí está la base de datos SQLite de mi antigua aplicación. Lee sus
proyectos y tareas y créalos en Mindwtr: las listas como proyectos,
conserva fechas límite y notas, omite las tareas completadas.
```

El agente deduce el formato de origen por sí mismo, asigna los campos al modelo de datos de Mindwtr y crea áreas, proyectos y tareas con las mismas herramientas que una captura normal. Sin script de conversión y sin renombrar columnas: esta vía cubre aplicaciones que no tienen importador ni una exportación limpia.

Tres notas prácticas:

- Haz primero una [copia de seguridad](/es/data-sync/backup-restore). Las escrituras por MCP siguen el modelo de datos normal de Mindwtr, pero en esta vía no hay vista previa de importación ni deshacer directo; la recuperación se hace restaurando un snapshot.
- Migra primero un proyecto pequeño y revisa el resultado antes de entregar la base de datos completa.
- El contenido de la exportación pasa por el modelo de IA que utilices; comprueba antes que la política de datos de tu proveedor te resulta aceptable para tareas privadas.
...
2. Exporta un CSV, renombra sus columnas con los nombres del [CSV de Mindwtr](/es/import/mindwtr-csv) e impórtalo de forma nativa. Es la alternativa con mayor fidelidad.
3. Prueba a copiar y pegar o usa texto sin formato para las tareas activas que aún necesites.
4. Usa el script auxiliar para CSV si quieres texto de adición rápida que puedas retocar a mano antes de importarlo.
5. Usa la API local, la CLI o MCP si necesitas una migración estructurada personalizada.
