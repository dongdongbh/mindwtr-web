...
iOS 小组件无法显示文本框或收集窗口：小组件按钮只能打开应用，或运行一个没有界面的操作。要获得类似 Android 的体验，即一次点按后在当前应用上方弹出一个小提示框，只需创建一次这个快捷指令：

1. 在**快捷指令**中新建一个快捷指令，命名为“记录”。
2. 添加**要求输入**（类型为文本，提示语为“任务”）。在提示框中点按麦克风即可改为语音听写。
3. 添加 Mindwtr 的 **Add to Mindwtr** 操作，并把输入传给 **Task**。这里支持快速添加语法（`/due:tomorrow`、`@phone`、`#tag`）。
4. 把它放到小组件所在的位置：在快捷指令详情中选择**添加到主屏幕**，即可在小组件旁边得到一个图标；**设置 → 操作按钮**可用侧边按钮运行；iOS 18 可将其加入控制中心；**设置 → 辅助功能 → 触控 → 轻点背面**可通过双击手机背面运行。

运行时会弹出一个小文本提示框，保存后返回你正在做的事。Mindwtr 保持关闭，任务会在下次打开应用时创建到收集箱，和所有 **Add to Mindwtr** 记录一样。如果想在保存前检查任务，请改用 **Capture to Mindwtr**。
