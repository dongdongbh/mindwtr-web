...
In the next release, this prompt also offers **Save & edit**. It saves the follow-up and opens the task editor so you can add dates, set an optional priority, or change its status to Inbox. By default, the new task is a Next action in the same project and section; explicit quick-add commands still apply. **Add next action** keeps the current quick-save behavior.
