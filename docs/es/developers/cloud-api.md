...
`assignedTo` es el nombre de una sola persona como cadena, por ejemplo `"Alex"`, no un array de identificadores de personas. Usa `null` o `""` para quitar la asignación. La creación y actualización de tareas por REST rechazan arrays, objetos, números y booleanos con HTTP 400. La búsqueda ignora las asignaciones ya almacenadas con un tipo incorrecto sin reescribirlas.
