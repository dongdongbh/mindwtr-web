...
Mindwtr 的設計是**預設簡單，需要時強大，生活忙亂時也寬容**。我們著重降低認知負擔、去除贅餘，並讓使用者保持專注。每項貢獻都會依下列原則衡量：
...
- **生活忙亂時也寬容**：絕不因為使用者當時力有未逮而讓他們愧疚。離開一段時間後回來必須輕鬆，大量積壓交給一次有邊界、做得完的處理，長期沒有動靜的項目是重新考慮的邀請，而且沒有連續打卡，也沒有生產力評分。
...
- [Bun](https://bun.sh/) — 套件管理員與執行階段。請使用 `.bun-version` 中固定的版本（目前為 1.3.5）或更新版本
- [Node.js](https://nodejs.org/) — JavaScript 執行階段（部分工具使用）。需要 20 或更新版本；`apps/mcp-server` 會發布至 npm 並宣告了該下限
