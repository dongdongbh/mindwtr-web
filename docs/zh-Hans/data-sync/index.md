...
在 Android 和 iOS 上，上述触发方式有两点不同。离开应用时 Mindwtr 会同步一次；返回应用时，如果距上次同步已超过 30 秒，会再同步一次。对于 WebDAV、自托管的 Mindwtr Cloud、Dropbox 和 iCloud，应用还会注册一个计划后台任务，系统最多每 15 分钟运行一次，应用关闭时也会运行；文件同步从不注册该任务，只在应用打开时同步。每次自动同步之后，应用至少要等待本次同步耗时的九倍，才会开始下一次（最短 5 秒，最长 5 分钟），因此数据量大时只是同步得更少，而不是持续不断地同步。
...
- **关闭不需要的自动同步触发方式**：目前还做不到；如果你希望有一个针对计划任务的设置，请在 GitHub 上告诉我们。
