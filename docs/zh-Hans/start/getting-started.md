...
全新安装后，首个选项是**开始使用 Mindwtr**。它会以你的语言创建**快速上手**项目并立即打开。你也可以导入现有任务、连接同步，或跳过引导。记录任务不需要先配置同步或导入。

手机和桌面端的快速上手项目都提供直接记录到**收集箱**、打开收集箱进行处理，以及打开**专注**的操作入口。项目、专注和日期的简短提示会出现在相关位置。关闭提示后，仍可通过**帮助**再次展开，并阅读对应章节的指南。提示的关闭状态仅保存在当前设备，不会与任务一起同步。

之后启动时，Mindwtr 默认打开**专注**。需要快速放下一个想法时，随时把新工作记录到**收集箱**。
...
[Mindwtr 视频演示播放列表](https://youtube.com/playlist?list=PLLwV6zeTfB_k)中有两个英文教程，完整演示从记录到每周回顾的全流程，也包括设置与同步。

- [Mindwtr Desktop Demo & Tutorial — Complete Local-First GTD Workflow](https://youtu.be/c-1Fxx5gFlo)（约 23 分钟）
- [Mindwtr Mobile Demo & Tutorial: Complete Local-First GTD Workflow](https://youtu.be/FApCFhViwJE)（约 26 分钟）
...
**实时预览**：在桌面端或手机端的任意收集输入框中输入时，输入框下方会用标签块显示 Mindwtr 识别到的内容：解析出的日期、项目、领域、情境、标签、分配给的人以及状态；若某个日期命令无法识别，还会出现一个警告标签块。在识别到内容之前标签块不会出现；它们与保存时使用的是同一次解析，因此任务会完全按照标签块显示的内容创建。
