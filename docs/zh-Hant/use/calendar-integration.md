...
宣告自身顏色的來源會保留這些顏色：Mindwtr 會讀取標準的 `COLOR` 屬性、Apple 的行事曆顏色，以及依類別設定的顏色（例如 Fossify 的），因此訂閱的行事曆看起來與它來源 app 中一樣。你在 Mindwtr 中自行選擇的顏色一律優先於來源提供的顏色；沒有任何顏色資訊的行事曆會取得自動指派且固定不變的顏色。
