...
Flathub Beta erfordert den Beta-Branch und Berechtigungen in `flathub/tech.dongdongbh.mindwtr`. Die AUR-Veröffentlichung erfolgt über einen separaten manuellen Workflow, der durch das GitHub Environment `aur-publish` geschützt ist. Prüfen Sie den gespeicherten Vorschlag, genehmigen Sie das Environment-Deployment und veröffentlichen Sie erst, wenn das AUR Pushes annimmt. AUR-Wartung verzögert nur diesen Kanal und lässt das Mindwtr-Release nicht fehlschlagen.
