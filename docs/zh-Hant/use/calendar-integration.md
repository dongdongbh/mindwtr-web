...
如水支援雙向的行事曆整合用途：將外部行事曆讀入規劃工具，以及在平台提供可寫入裝置行事曆時，將如水任務推送至該行事曆。

- **行動版（iOS/Android）：**系統已公開的裝置行事曆、ICS 訂閱 URL，以及如水 -> 裝置行事曆的單向推送
- **桌面版（macOS）：**透過 EventKit 讀取 Apple 行事曆並單向推送任務，另支援 ICS 訂閱 URL
- **桌面版（Linux/Windows）與網頁版：**ICS 訂閱 URL

## 概念

- **硬性時程：**外部行事曆中的會議／課程。
- **彈性時程：**以 `startTime` 與 `timeEstimate` 排程的如水任務。
...
如水的重複任務會產生任務實例，而不是展開成行事曆系列：

- 使用中的任務實例有截止日期或排定開始時間時，會顯示在行事曆中。
- 如水預設不會在行事曆中預先建立未來任務。只有完成目前的重複任務後，才會建立下一項真正的任務實例。
- 若為重複任務啟用**在行事曆顯示下一次發生時間**，行事曆可顯示一項僅供規劃的下次發生預覽。該預覽並非真正的任務；行動版／macOS 行事曆推送會將它寫成一般單一事件，而不是原生重複事件。
- **固定排程**會維持固定的排程節奏。每月 1 日截止的任務會固定在規劃週期上，但如水仍然只會在每次完成後建立下一個實例，不會填滿未來每個月份。
- **完成後重複**會依目前任務的完成時間計算下一個實例。例如，一項一般每月習慣若在 15 日完成，下次就會以 15 日為基準排程。
- 行動版與 macOS 行事曆推送會反映這些具體任務實例，不會將如水重複規則匯出為原生重複行事曆事件。
...
3. 設定開始與結束時間。如水會比對已排程任務及可見外部事件，檢查時段。
...
1. 從日檢視開啟**行事曆 -> 排程任務**。
2. 選擇現有任務。
3. 如水會避開可見外部事件與已排程任務，尋找當天最早的空閒時段。

行動版排程面板也採用相同方式：查看某一天時選擇尚未排程的工作，再指派具體開始時間。

如有 `timeEstimate`，如水會以它作為預設持續時間。若發生衝突，請選擇其他時間或縮短持續時間。
...
| 平台 | 支援的行事曆功能 | 注意事項 |
| --- | --- | --- |
| iOS/Android 行動版 | 將如水任務推送至裝置行事曆 | Android 已透過 Google Calendar 驗證。iOS 請使用已提供給 Apple 行事曆／EventKit 的行事曆。 |
...
| macOS 桌面版 | 將如水任務推送至 Apple 行事曆 | 透過 EventKit，將已排程／到期的如水任務寫入所選的可寫入 Apple 行事曆。 |
...
- 在如水內進行行事曆供應商 OAuth，例如從如水登入 Google、Microsoft 或 Nextcloud。
- 回傳 `HTTP 401` 的須驗證／私人 URL，除非行事曆供應商已將密鑰嵌入 URL。
- 行事曆網頁 URL，包括顯示 HTML 而非原始 `.ics` 資料的公開分享頁面。
- 從如水編輯外部行事曆事件。
- 透過如水同步外部行事曆事件。外部事件只會在本機擷取並快取。
- 任務／行事曆雙向同步。推送的行事曆事件由如水任務產生。
- 將重複任務規則匯出為原生重複行事曆事件。
...
在 iOS 與 Android 上，如水可將排定時間或設有截止日期的任務推送至所選裝置行事曆：
...
- 推送至專用的 `Mindwtr` 行事曆時，如水會保留任務標題。如果選擇共用行事曆，推送事件標題會加上 `Mindwtr: ` 前綴，以便與一般事件區分。
- 任務描述會成為事件筆記，任務地點會成為事件地點。
- 如果選擇名為 `Mindwtr` 的專用行事曆，行事曆應用程式可以用該行事曆本身的色彩顯示如水項目。
...
4. 在 Google Calendar Android 應用程式中啟用**與其他應用程式分享 Google Calendar 資料**，讓 Android 將 Google 行事曆公開給如水。
5. 返回如水的**設定 → 行事曆**，點選**重新整理行事曆**，再選擇顯示你 Google 帳號的 `Mindwtr` 目標。

如果 Google 帳號支援的 `Mindwtr` 行事曆尚未出現在如水中，代表 Android 尚未透過系統行事曆供應程式將它公開。請重新整理 Google Calendar、檢查 Android 帳號同步、在 Google Calendar 啟用**與其他應用程式分享 Google Calendar 資料**，再於如水點選**重新整理行事曆**。
...
如果目標清單中沒有 `Mindwtr` 行事曆，請先確認它在 Apple 行事曆中可見，再返回如水並點選**重新整理行事曆**。

### 行動版：讀取裝置行事曆

在行動版中，如水可從裝置行事曆資料庫讀取行事曆：

- **Android：**透過 Android 行事曆供應程式。若同步應用程式未透過該供應程式公開行事曆，如水就無法看見。
...
如水會保持唯讀，不會對行事曆來源執行供應商 OAuth。

如水會在讀取清單中隱藏自己推送的 `Mindwtr` 行事曆，以免匯入自己建立事件的重複副本。

### macOS：Apple 行事曆整合

在 macOS 桌面版中，如水可透過 EventKit 讀取 Apple 行事曆事件，並推送已排程／到期的如水任務：

1. 開啟**設定 -> 行事曆**
2. 要求 Apple 行事曆存取權
3. 在 macOS **系統設定 -> 隱私權與安全性 -> 行事曆**中允許如水
4. 若想將如水任務寫入 Apple 行事曆，啟用**將任務推送至行事曆**
5. 選擇專用的 `Mindwtr` 行事曆或其他可寫入行事曆目標

此功能只適用於 Apple 行事曆中已可見的行事曆。目前 Linux 與 Windows 沒有原生桌面行事曆帳號整合。
...
如水預期 URL 會擷取原始 iCalendar 文字。可用的 feed 通常：
...
如果如水顯示 `HTTP 401`，代表伺服器要求驗證。行事曆 URL 不支援使用者名稱／密碼提示、CalDAV 登入或 bearer-token 標頭。請改用供應商提供的秘密 iCalendar 訂閱 URL。
...
- 桌面版與行動版的「行事曆」可從外部事件建立另一項如水任務。如水會在可用時複製事件標題、日期／時間、地點、描述與行事曆名稱。
- 外部行事曆在如水中為**唯讀**。從事件建立任務不會修改原始事件。
- ICS 重複事件支援 `FREQ=DAILY`、`WEEKLY`、`MONTHLY` 與 `YEARLY`，包括如水可展開至可見範圍之模式的 `INTERVAL`、`COUNT`、`UNTIL`、`BYDAY`、`BYMONTH` 與 `BYMONTHDAY`。
