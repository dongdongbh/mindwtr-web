...
Flathub beta 需要 `flathub/tech.dongdongbh.mindwtr` 中的 beta 分支和权限。AUR 发布使用单独的手动工作流，并受 GitHub `aur-publish` Environment 保护。请审核保存的提案、批准 Environment 部署，并仅在 AUR 恢复推送后发布。AUR 上游维护只会延迟该渠道，不会使 Mindwtr 发布失败。
