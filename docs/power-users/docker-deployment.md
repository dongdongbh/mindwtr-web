...
4. **Access the services**:
   ...
   - **Cloud liveness check:** Open `http://localhost:8787/health`.
   - **Cloud storage readiness:** Open `http://localhost:8787/ready`.
...
This uses Caddy's internal certificate authority. Each client device must trust Caddy's local root certificate before Mindwtr will accept the HTTPS connection. Mindwtr 1.2.0 and later trusts user-installed certificate authorities on Android; iPhone and iPad users must also install and explicitly enable full trust for the root certificate. Public Let's Encrypt certificates remain the simplest option when the hostname can be reached publicly.
...
For a file-backed Docker secret, remove `MINDWTR_CLOUD_AUTH_TOKENS` from your `.env` file, put the token in a host file only its owner can read, and start Compose with the `docker/compose.secrets.yaml` overlay. `MINDWTR_CLOUD_AUTH_TOKENS_FILE_HOST` must be an absolute host path:
...
The overlay mounts that file read-only at `/run/secrets/mindwtr_cloud_tokens` and sets `MINDWTR_CLOUD_AUTH_TOKENS_FILE` for you, so the token bytes never reach the rendered Compose environment. Keep the host file at mode `0600`. The server still refuses to start when neither the inline setting nor a readable token file provides a valid token.

The same overlay works with the Caddy HTTPS stack. Remove the inline token from `docker/.env.https.local`, then run:
...
A fresh browser opening the PWA can arrive with the Cloud URL already filled in, so people only enter their token:

- **Same domain (recommended):** when the app and the cloud API share one domain (the Caddy HTTPS setup does this), the web app detects the cloud on its own origin automatically. Nothing to configure.
- **Split domains:** set `MINDWTR_DEFAULT_CLOUD_URL` on the `mindwtr-app` container to the public cloud URL, for example in Kubernetes or any deployment where the app and API live on different hosts.
