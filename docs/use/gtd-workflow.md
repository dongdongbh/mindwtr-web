...
4. **Upcoming** previews `next` tasks the deferral currently holds back but which surface within the next 7 days — a future start date, or a recurring task waiting for its next due or review date. It also lists a task whose start time is later the same day, which Focus hides until that time. Rows are sorted by the day they will appear and show that date. A row starting later today can be starred into Today's Focus — the star holds and the task resurfaces there at its start time — while a row deferred to another day cannot. The section disappears when nothing is coming.
